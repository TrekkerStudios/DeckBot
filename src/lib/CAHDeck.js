export class CAHDeck {
  ...
  async _loadDeck(link) {
    if (typeof this.compactSrc != "undefined") {
      let json;
      if (link) {
        json = await fetch(this.compactSrc).then((data) => data.json());
      } else {
        json = JSON.parse(this.compactSrc);
      }
      this.deck = this._hydrateCompact(json);
    } else if (typeof this.fullSrc != "undefined") {
      let json;
      if (link) {
        json = await fetch(this.fullSrc).then((data) => data.json());
      } else {
        json = JSON.parse(this.fullSrc);
      }
      this.deck = json;
    } else {
      throw Error(
  ...
  static async fromCompact(compactSrc, link) {
    let n = new CAHDeck();
  ...
  static async fromFull(fullSrc, link) {
    let n = new CAHDeck();
    n.fullSrc = fullSrc;
    await n._loadDeck(link);
    return n;
